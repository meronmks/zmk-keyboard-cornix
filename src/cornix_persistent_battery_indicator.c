 ...
 * color whenever the local battery or USB state changes. The widget retains
 * control while charging and at critical battery level so its pulse/blink
...
static void update_indicator(struct k_work *work) {
    ...
    /* Preserve the widget's charging pulse and critical-battery warning. */
